...
<script>
     ...
     function selectProduct($this){
           ...
           // Load branch stock for this product
           loadBranchStock(productId);
         }

     function loadBranchStock(productId) {
         fetch(`/user/branch-stock/${productId}`)
             .then(response => response.json())
             .then(data => {
                 displayBranchStock(data);
             })
             .catch(error => {
     ...
     function displayBranchStock(stockData) {
         ...
         // Get user's branch name from API response
         const branchName = stockData.branch_name || 'Your Branch';
         ...
         row.innerHTML = `
             ...
             <td>
                 <input type="number"
                        ...
                        name="damage_qty[${branchId}]"
                        min="0"
                        max="${stockData.quantity}"
                        value="0"
                        onchange="updateRemainingStock(this, ${stockData.quantity})"
                        style="width: 80px;">
             </td>
             <td>
                 <span class="remaining-stock">${stockData.quantity}</span>
             </td>
         `;
